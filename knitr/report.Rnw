...
\title{Analysis of names from newborn children in Zurich}
...
In this document we will give an example of dynamic reporting using a data set containing the names of newborn children in the city of Zurich since the year 1993. The data were downloaded from \url{https://data.stadt-zuerich.ch/dataset/bev_vornamen\_baby\_od3700}.

\section*{Import data and perform manipulations}
...
ggplot(data = datCleaned, aes(x = year, y = births, color = sex)) +
    ...
    geom_label(aes(label = name), alpha = 0.8, show.legend = FALSE, 
               size = 2) +
